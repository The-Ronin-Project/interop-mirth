...
class AppointmentLoadTest {
    ...
    @BeforeEach
    fun setup() {
        channel = AppointmentLoad(mockk(), mockk(), mockk(), mockk(), mockk())
    }
        ...
        verify {
            kafkaDagPublisher.publishDag(
                withArg { resourceType ->
                    assertEquals(ResourceType.Appointment, resourceType)
                },
                withArg { consumedResources ->
                    assertEquals(consumedResources.size, 1)
                    assertTrue(consumedResources.contains(ResourceType.Patient))
                },
            )
        }
    }
    ...
    fun `publish events honor batch size override with matching resource type`() {
        ...
        val appointmentPublish: AppointmentPublish = mockk()
        ...
        every {
            kafkaPublishService.retrievePublishEvents(
                ...
                DataTrigger.NIGHTLY,
                "interop-mirth-appointment_group",
            )
        } returns
            listOf(
